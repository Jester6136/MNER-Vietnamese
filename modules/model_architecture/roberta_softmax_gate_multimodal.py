...
class RobertaSoftmaxGateMultimodal(RobertaPreTrainedModel):
    ...
    def __init__(self, config, layer_num1=1, layer_num2=1, layer_num3=1, num_labels_=2, auxnum_labels=2):
        super(RobertaSoftmaxGateMultimodal, self).__init__(config)
    ...
    def forward(self, input_ids, segment_ids, input_mask, added_attention_mask, visual_embeds_mean, visual_embeds_att, labels=None):
        ...
        final_output = torch.cat((sequence_output, gated_converted_att_vis_embed), dim=-1)
...
if __name__ == "__main__":
    model = RobertaSoftmaxGateMultimodal.from_pretrained('vinai/phobert-base-v2')
    print(model)